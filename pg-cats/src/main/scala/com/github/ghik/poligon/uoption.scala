...
object uoption {
  ...
  object UNone {
    private[uoption] val wrap: WrappedNone =
      new WrappedNone(1, this)
...
object testity {
  ...
  def main(args: Array[String]): Unit = {
    val us = USome(USome(123))
    val opt: Any = Opt(Opt(123))

    opt match {
      case Opt(v) => println(s"yes $v")
    }
    opt match {
      case Opt(Opt(v)) => println(s"yes $v")
    }
    opt match {
      case Opt(Opt(Opt(v))) => println(s"yes $v")
    }
  }
}
